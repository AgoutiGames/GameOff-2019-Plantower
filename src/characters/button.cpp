...
void Button::init() {
    ...
    put(m_id, "id");
    std::string sound_path = "EMPTY";
    put(sound_path, "sound");
    if(sound_path != "EMPTY") {m_clicked = m_scene->get_audio_manager().get_sound(sound_path);}
}
    ...
    for(salmon::CollisionRef& c : get_collisions()) {
        if(c.mouse()) {
            ...
            if(mouse.left.pressed) {
                if(m_clicked.good()) {m_clicked.play();}
